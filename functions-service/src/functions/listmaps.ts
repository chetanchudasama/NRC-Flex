...
/**
 * public twilio function exposed on the /listmaps path.
 * protected via flex token (any user).
 * returns an object with a JSON array of the maps that hold callbacks for this system.
 * @param {*} context
 * @param {*} event
 * @param {*} callback
 */
...
export const handler: ServerlessFunctionSignature = TokenValidator(
  async function (
    ...
    try {
      const callbackMapResult = await client.sync
      ...
      const callbackMaps =
        callbackMapResult &&
        callbackMapResult.data &&
        callbackMapResult.data.callbackMaps
          ? callbackMapResult.data.callbackMaps
          : [];

      response.setStatusCode(200);
      response.setBody({ callbackMaps });
      callback(null, response);
    } catch (e) {
      //bad
      callback(e);
