...
export const handler: ServerlessFunctionSignature = TokenValidator(
  async function (
    ...
    if (
      ...
      event.MapItemId === undefined ||
      event.TokenResult === undefined
    ) {
      response.setBody("Missing fields in event for complete callback");
    ...
    if (context.SYNC_SERVICE_SID === undefined) {
      response.setBody("Missing fields in context for complete callback");
    ...
    if (goodResult) {
      ...
      response.setStatusCode(500);
      response.setBody("Failed to complete callback");
      callback(null, response);
    }
  }
);
...
async function fetchAndRemoveMapItem(
  ...
  while (success === false && attempts <= 3) {
    ...
    try {
      ...
      mapItem.data.completed = true;
      mapItem.data.completedBy = workerId;
      mapItem.data.completedAt = new Date().toISOString();
